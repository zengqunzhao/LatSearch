...
def fetch_image(ele: dict[str, str | Image.Image], size_factor: int = IMAGE_FACTOR) -> Image.Image:
    ...
    if image_obj is None:
        raise ValueError(
            "Unrecognized image input. Expected a local path, HTTP URL, base64 image, "
            f"or PIL.Image; got {image}"
        )
        ...
        ele (dict): a dict contains the configuration of video.
            support either `fps` or `nframes`:
                ...
                - fps: the fps to extract frames for model inputs.
                    - min_frames: minimum frames, used only when fps is provided.
                    - max_frames: maximum frames, used only when fps is provided.
...
def _read_video_torchvision(
    ...
    if version.parse(torchvision.__version__) < version.parse("0.19.0"):
        if "http://" in video_path or "https://" in video_path:
            warnings.warn(
                "torchvision < 0.19.0 does not support HTTP video paths; "
                "upgrade to torchvision >= 0.19.0."
            )
...
# This function is from EvoSearch paper
def _read_video(ele: dict, video_fps: int = 16) -> torch.Tensor:
    """sample video frames from a video tensor"""
    video = ele["video"]
...
def _read_video_decord(
    ...
    if ele["sample_type"] == "uniform":
        ...
        for pt in pts:
            idx.extend(frames_idx[pt - frames_each_pts // 2 : pt + frames_each_pts // 2])
    video = vr.get_batch(idx).cpu().numpy()
