...
def _insert_adapter_name_into_state_dict(state_dict, adapter_name: str, parameter_prefix: str):
    """Insert the adapter name into PEFT state-dict keys."""
...
# VideoReward checkpoint keys differ across Transformers releases.
def remap_checkpoint_keys(ckpt_dict, target_keys=None):
    """Map legacy VideoReward keys only when the target model requires it."""
    ...
    for k, v in ckpt_dict.items():
        ...
        if k.startswith("base_model.model.visual."):
            candidates.append(
                k.replace("base_model.model.visual.", "base_model.model.model.visual.")
            )
        elif k.startswith("base_model.model.model.") and not k.startswith(
        ...
        new_k = next((candidate for candidate in candidates if candidate in target_keys), k)
...
def load_model_from_checkpoint(model, checkpoint_dir, checkpoint_step):
    ...
    if checkpoint_step is None or checkpoint_step == -1:
        ...
        if checkpoint_path not in checkpoint_paths:
            ...
            print(
                ...
                f"using the latest checkpoint: {checkpoint_path}"
            )
        else:
            print(
                f"===> Checkpoint step {checkpoint_step} found, "
                f"using the specified checkpoint: {checkpoint_path}"
            )
    ...
    if os.path.exists(full_ckpt):
        ...
        model_state_dict = remap_checkpoint_keys(model_state_dict, model_dict)
        ...
        if not filtered_state_dict:
            raise RuntimeError(f"No compatible weights found in {full_ckpt}")
        print(
            f"===> Loading {len(filtered_state_dict)}/{len(model_state_dict)} "
            "VideoReward checkpoint tensors"
        )
