...
def freq_mix_noise_3d(x, noise, LPF, output_type="real"):
    """
    Args:
        x: diffused latent
    ...
    # frequency filtering
    x_freq_mixed = x_freq * LPF + noise_freq * (1 - LPF.square()).sqrt()

    # IFFT
    x_freq_mixed = fft.ifftshift(x_freq_mixed, dim=(-3, -2, -1))
    x_mixed = fft.ifftn(x_freq_mixed, dim=(-3, -2, -1))

    if output_type == "real":
        res = x_mixed.real
    elif output_type == "+":
        res = x_mixed.real + x_mixed.imag
    elif output_type == "-":
        res = x_mixed.real - x_mixed.imag
    else:
        raise ValueError(f"Expected: real, +, -. But got {output_type}")
    return res
