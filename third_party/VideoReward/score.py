...
import pandas as pd
import torch
from third_party.VideoReward.vision_process import process_vision_info
from third_party.VideoReward.data import DataConfig
from third_party.VideoReward.utils import ModelConfig, PEFTLoraConfig, TrainingConfig
from third_party.VideoReward.utils import load_model_from_checkpoint
from third_party.VideoReward.train_reward import create_model_and_processor
from third_party.VideoReward.prompt_template import build_prompt
...
class VideoVLMRewardInference():
    ...
    def __init__(self,
        ...
        model, processor, peft_config = create_model_and_processor(model_config=model_config,
                                                                   peft_lora_config=peft_lora_config,
                                                                   training_args=training_args)
        model, checkpoint_step = load_model_from_checkpoint(model,
                                                            load_from_pretrained,
    ...
    def reward(self, video_tensors, prompts, fps=None, num_frames=None, max_pixels=None, use_norm=True):
        """
        Inputs:
            video_tensors: Tensor, B video tensors.
        ...
        batch = self.prepare_infer_batch(video_tensors, prompts, fps, num_frames, max_pixels)
        rewards = self.model(
            return_dict=True,
            **batch
        )["logits"]
